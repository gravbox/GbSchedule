...
Namespace Gravitybox.Objects
	...
	<Serializable()> _
	Public Class RecurrenceYear

#Region "Class Members"

		'Private Constants
    Friend Const MyXMLNodeName As String = "recurrenceyear"
		...
		Private Const m_def_MonthOrdinal As Integer = 1
		...
		Private m_RecurrenceMode As RecurrenceSubTypeConstants = m_def_RecurrenceMode
		Private m_MonthInterval As Integer = m_def_MonthInterval
		Private m_DayInterval As Integer = m_def_DayInterval
		Private m_DayPosition As RecurrenceOrdinalDayConstants = m_def_DayPosition
		Private m_DayOrdinal As RecurrenceOrdinalConstants = m_def_DayOrdinal
		Private m_MonthOrdinal As Integer = m_def_MonthOrdinal
...
#Region "Property Implementation"
		...
		<Description(""), _
		Browsable(True)> _
		Public Property MonthOrdinal() As Integer
			Get
				If m_MonthOrdinal < Recurrence.RecurrenceNumericMinimum Then m_MonthOrdinal = Recurrence.RecurrenceNumericMinimum
				If m_MonthOrdinal > Recurrence.RecurrenceNumericMaximum Then m_MonthOrdinal = Recurrence.RecurrenceNumericMaximum
				Return m_MonthOrdinal
			End Get
			Set(ByVal Value As Integer)
				Try
					If Value < Recurrence.RecurrenceNumericMinimum Then Value = Recurrence.RecurrenceNumericMinimum
					If Value > Recurrence.RecurrenceNumericMaximum Then Value = Recurrence.RecurrenceNumericMaximum
					If m_MonthOrdinal <> Value Then
						m_MonthOrdinal = Value
						OnRefresh(Me, New System.EventArgs)
					End If
				Catch ex As Exception
					Call ErrorModule.SetErr(ex)
...
#Region "XML"
		...
		Friend Function FromXML(ByVal xml As String) As Boolean
			...
			Try
				...
				m_MonthOrdinal = GetIntlInteger(GetNodeValue(XMLDoc, startXPath & "monthordinal", m_MonthOrdinal.ToString))
		...
		Friend Function ToXML() As String
			...
			Try
				...
				Call xmlHelper.addElement(parentNode, "monthordinal", GetIntlInteger(m_MonthOrdinal).ToString)
