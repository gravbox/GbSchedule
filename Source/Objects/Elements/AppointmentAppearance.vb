...
Namespace Gravitybox.Objects
  ...
  <Serializable(), _
  DesignerSerializer(GetType(Gravitybox.Design.Serializers.AppointmentAppearanceSerializer), GetType(CodeDomSerializer)), _
...
#Region "Class Members"
    ...
    Private ReadOnly startXPath As String = "//" & MyXMLNodeName & "/"
		...
		Private Const m_def_Transparency As Integer = 0
		Private Const m_def_IsRound As Boolean = False
		Private Const m_def_ShadowSize As Integer = 0
		Private Const m_def_Ghosted As Boolean = False

		'Private Member Variables
		Private m_Transparency As Integer = m_def_Transparency
		Private m_IsRound As Boolean = m_def_IsRound
		Private m_ShadowSize As Integer = m_def_ShadowSize
		Private m_Ghosted As Boolean = m_def_Ghosted
...
#Region "Clone"
		...
		Friend Overrides Function CloneMe() As Object
			Try
				Dim element As New Gravitybox.Objects.AppointmentAppearance
...
#Region "Property Implementations"
    ...
    ''' <summary>
    ''' Determines the level of transparency for this appointment.
    ''' </summary>
    <Browsable(True), _
    ...
    Description("Determines the level of transparency for this appointment.")> _
    ...
    ''' <summary>
    ''' Determines if this appointment has rounded edges.
    ''' </summary>
    <Browsable(True), _
    Category("Appearance"), _
    DefaultValue(m_def_IsRound), _
    Description("Determines if this appointment has rounded edges.")> _
    Public Property IsRound() As Boolean
      Get
        Return m_IsRound
      End Get
      Set(ByVal Value As Boolean)
        If m_IsRound <> Value Then
          m_IsRound = Value
          OnRefresh(Me, New AfterBaseObjectEventArgs(Me))
        End If
      End Set
    ...
    ''' <summary>
    ''' Determines if the size of the appointment shadow.
    ''' </summary>
    <Browsable(True), _
    Category("Appearance"), _
    DefaultValue(m_def_ShadowSize), _
    Description("Determines if the size of the appointment shadow.")> _
    ...
    ''' <summary>
    ''' Determines if the appointment is ghosted for copy and paste operations.
    ''' </summary>
    <Browsable(True), _
    Category("Appearance"), _
    DefaultValue(m_def_Ghosted), _
    Description("Determines if the appointment is ghosted for copy and paste operations.")> _
    Public Property Ghosted() As Boolean
      Get
        Return m_Ghosted
      End Get
      Set(ByVal Value As Boolean)
        If m_Ghosted <> Value Then
          m_Ghosted = Value
          OnRefresh(Me, New AfterBaseObjectEventArgs(Me))
        End If
      End Set
...
#Region "XML"
    ...
    Public Overloads Overrides Function FromXML(ByVal xml As String) As Boolean
      ...
      Try
        ...
        m_IsRound = Boolean.Parse(GetNodeValue(XMLDoc, startXPath & "isround", m_IsRound.ToString()))
        m_ShadowSize = GetIntlInteger(GetNodeValue(XMLDoc, startXPath & "shadowsize", m_ShadowSize.ToString()))
        m_Ghosted = Boolean.Parse(GetNodeValue(XMLDoc, startXPath & "ghosted", m_Ghosted.ToString()))
        MyBase.FromXML(xml)
        Return True
    ...
    End Function
		...
		Friend Overrides Function XmlNode() As System.Xml.XmlNode
			...
			Try
				...
				Call xmlHelper.addElement(parentNode, "isround", m_IsRound.ToString())
				Call xmlHelper.addElement(parentNode, "shadowsize", m_ShadowSize.ToString())
				Call xmlHelper.addElement(parentNode, "ghosted", m_Ghosted.ToString())
